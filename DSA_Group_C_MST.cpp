...
public:
    ...
    void primsMST()
    {
        Graph MST(v);//object of class Graph is MST where we pass argument vertex 
        vector<bool> setMST(v, false);  // show that is vetex is included in MST or not
        vector<int> weight(v, infinity); // store the minimum weight required to reach vertex
